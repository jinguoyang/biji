...
name_web_retry = "web:retry:{0}".format(options.server_port)
# pattern = re.compile(
#     r'^http://'
...
class WebRequest(object):
    ...
    def dismiss_room(self, table):
        # print 'enter dismiss room',table.conf.is_aa()
        if table.conf.is_aa():
            if table.cur_round <= 1:
                # print 'aaaaa',',table:',table,',player:', table.player_dict.values()
                for p in table.player_dict.values():
                    # print 'bbbbb'
                    self.aa_refund(p.uuid, 0)
                    # elif table.cur_round < table.conf.max_rounds:
                    #     for p in table.player_dict.values():
    ...
    def aa_refund(self, userid, refund_type):
        ...
        body = proto_web.SerializeToString()
        # print 'aa userid:', userid, ',refund_type:', refund_type, 'url:',url
        self.post(url, body)
    ...
    def aa_cons(self, userid):
        ...
        # print 'aa_cons userid: appid', userid, self.app_id, url
        body = proto_web.SerializeToString()
        self.post(url, body)
    ...
    def sync_data(self, data):
        # 发送积分同步
        url = self.web_record_url + "/sync_data"
        body = json.dumps(data)
        self.post(url, body)
